...
%% ============================================================
% RESUMEN Y VISUALIZACIÓN DE BIOMARCADORES DE LA ONDA P
%
% Este script carga los resultados del análisis morfológico de la onda P
% y genera tablas resumen, figuras de onda P media, mapas de correlación
% y diagramas de caja de los biomarcadores calculados.
%
% Entrada:
%   - ondasP_features_completas.mat
%
% Salidas:
%   - features_ondaP.xlsx
%   - tablas_correlaciones_ondaP.xlsx
%   - correlaciones_ondaP.mat
%   - figuras de onda P media por grupo
...
%
% Requisitos:
%   - Resultados generados previamente por el script de análisis de onda P.
%% ============================================================
...
carpeta_boxplots = fullfile(carpeta_out, 'boxplots_correlaciones');
if ~exist(carpeta_boxplots, 'dir')
    mkdir(carpeta_boxplots);
end

carpeta_heatmaps = fullfile(carpeta_out, 'heatmaps_correlacion');
if ~exist(carpeta_heatmaps, 'dir')
    mkdir(carpeta_heatmaps);
end

carpeta_pmedia = fullfile(carpeta_out, 'figuras_P_media');
if ~exist(carpeta_pmedia, 'dir')
    mkdir(carpeta_pmedia);
end

if ~exist(archivo_mat, 'file')
    error('No se encuentra el archivo %s. Ejecuta primero el script principal de onda P.', archivo_mat);
end
...
%% FIGURA DE ONDA P MEDIA POR GRUPO

if ~isempty(resultados_sanos) || ~isempty(resultados_parox)

    if ~isempty(resultados_sanos)
        n_muestras = numel(resultados_sanos(1).P_media);
    else
        n_muestras = numel(resultados_parox(1).P_media);
    end
    ...
    for i = 1:numel(resultados_sanos)
        if isfield(resultados_sanos(i), 'P_media') && ~isempty(resultados_sanos(i).P_media)
            Pmed_sanos = [Pmed_sanos; resultados_sanos(i).P_media(:)']; %#ok<AGROW>
        end
    end

    Pmed_parox = [];
    for i = 1:numel(resultados_parox)
        if isfield(resultados_parox(i), 'P_media') && ~isempty(resultados_parox(i).P_media)
            Pmed_parox = [Pmed_parox; resultados_parox(i).P_media(:)']; %#ok<AGROW>
        end
    end

    %% Escala común para ambos grupos

    valores_y = [Pmed_sanos(:); Pmed_parox(:)];
    ...
    if ~isempty(Pmed_sanos)
        plot(t_ms, Pmed_sanos', ...
            'Color', [0.75 0.75 0.75], ...
            'LineWidth', 0.6);

        plot(t_ms, mean(Pmed_sanos, 1, 'omitnan'), ...
            'k', ...
            'LineWidth', 2);
    end

    hold off
    title('P media - SANO', 'Interpreter','none');
    ...
    if ~isempty(Pmed_parox)
        plot(t_ms, Pmed_parox', ...
            'Color', [0.75 0.75 0.75], ...
            'LineWidth', 0.6);

        plot(t_ms, mean(Pmed_parox, 1, 'omitnan'), ...
            'k', ...
            'LineWidth', 2);
    end

    hold off
    title('P media - FA_PA_RS', 'Interpreter','none');
    ylabel('Amplitud (mV)', 'Interpreter','none', 'FontSize', 14);
    xlabel('Tiempo relativo (ms)', 'Interpreter','none');
    grid on
    box on
    ...
    sgtitle('Ondas P medias por paciente en cada grupo', 'Interpreter','none');

    saveas(f, fullfile(carpeta_pmedia, 'P_media_por_grupo.png'));
    close(f)
end
...
for i = 1:numel(resultados_sanos)-1
    for j = i+1:numel(resultados_sanos)
        ...
        if isfield(resultados_sanos(i), 'P_media') && isfield(resultados_sanos(j), 'P_media')
            correlacion_media_sanos(end + 1, 1) = corr(resultados_sanos(i).P_media', resultados_sanos(j).P_media');
        end

        if isfield(resultados_sanos(i), 'P_std') && isfield(resultados_sanos(j), 'P_std')
            correlacion_std_sanos(end + 1, 1) = corr(resultados_sanos(i).P_std', resultados_sanos(j).P_std');
        end
    end
end
...
for i = 1:numel(resultados_parox)-1
    for j = i+1:numel(resultados_parox)
        ...
        if isfield(resultados_parox(i), 'P_media') && isfield(resultados_parox(j), 'P_media')
            correlacion_media_parox(end + 1, 1) = corr(resultados_parox(i).P_media', resultados_parox(j).P_media');
        end

        if isfield(resultados_parox(i), 'P_std') && isfield(resultados_parox(j), 'P_std')
            correlacion_std_parox(end + 1, 1) = corr(resultados_parox(i).P_std', resultados_parox(j).P_std');
        end
    end
end
...
for i = 1:numel(resultados_sanos)
    for j = 1:numel(resultados_parox)
        ...
        if isfield(resultados_sanos(i), 'P_media') && isfield(resultados_parox(j), 'P_media')
            correlacion_cruzada_media(end + 1, 1) = corr(resultados_sanos(i).P_media', resultados_parox(j).P_media');
        end

        if isfield(resultados_sanos(i), 'P_std') && isfield(resultados_parox(j), 'P_std')
            correlacion_cruzada_std(end + 1, 1) = corr(resultados_sanos(i).P_std', resultados_parox(j).P_std');
        end
    end
end
...
% --- SANO vs SANO (P_media)
M_sanos_media = nan(numel(resultados_sanos));

for i = 1:numel(resultados_sanos)
    for j = 1:numel(resultados_sanos)
        M_sanos_media(i,j) = corr(resultados_sanos(i).P_media', resultados_sanos(j).P_media');
    end
end
...
colormap(jet)
caxis([-1 1])
title('Correlación entre pacientes SANO (P media)', 'Interpreter','none')
...
% --- FA_PA_RS vs FA_PA_RS (P_media)
M_parox_media = nan(numel(resultados_parox));

for i = 1:numel(resultados_parox)
    for j = 1:numel(resultados_parox)
        M_parox_media(i,j) = corr(resultados_parox(i).P_media', resultados_parox(j).P_media');
    end
end
...
colormap(jet)
caxis([-1 1])
title('Correlación entre FA_PA_RS (P media)', 'Interpreter','none')
xlabel('Paciente', 'Interpreter','none')
ylabel('Paciente', 'Interpreter','none', 'FontSize', 14)
saveas(f, fullfile(carpeta_heatmaps, 'heatmap_parox_media.png'));
close(f)

% --- SANO vs FA_PA_RS (P_media)
M_cruzada_media = nan(numel(resultados_sanos), numel(resultados_parox));

for i = 1:numel(resultados_sanos)
    for j = 1:numel(resultados_parox)
        M_cruzada_media(i,j) = corr(resultados_sanos(i).P_media', resultados_parox(j).P_media');
    end
end
...
colormap(jet)
caxis([-1 1])
title('Correlación SANO vs FA_PA_RS (P media)', 'Interpreter','none')
...
for i = 1:numel(resultados_sanos)
    for j = 1:numel(resultados_sanos)
        M_sanos_std(i,j) = corr(resultados_sanos(i).P_std', resultados_sanos(j).P_std');
    end
end

% --- FA_PA_RS vs FA_PA_RS (P_std)
M_parox_std = nan(numel(resultados_parox));

for i = 1:numel(resultados_parox)
    for j = 1:numel(resultados_parox)
        M_parox_std(i,j) = corr(resultados_parox(i).P_std', resultados_parox(j).P_std');
    end
end

% --- SANO vs FA_PA_RS (P_std)
M_cruzada_std = nan(numel(resultados_sanos), numel(resultados_parox));

for i = 1:numel(resultados_sanos)
    for j = 1:numel(resultados_parox)
        M_cruzada_std(i,j) = corr(resultados_sanos(i).P_std', resultados_parox(j).P_std');
    end
end
...
% Tabla con un valor de similitud por paciente para P_media y P_std.
...
title('Media de correlación de ondas P por paciente', 'Interpreter','none');
...
title('Desviación de correlación de ondas P por paciente', 'Interpreter','none');
ylabel('Correlación', 'Interpreter','none', 'FontSize', 14);
xlabel('Grupo', 'Interpreter','none');
grid on
...
title('Amplitud media de la onda P', 'Interpreter','none');
...
title('Variabilidad de amplitud de la onda P', 'Interpreter','none');
...
title('Variabilidad media de la morfología de la onda P', 'Interpreter','none');
...
% CorrGrupoMedia: P_media, un valor por paciente
f = figure('Visible','off');
...
title('Similitud de la onda P media con el resto del grupo', 'Interpreter','none');
...
% CorrGrupoStd: P_std, un valor por paciente
f = figure('Visible','off');
...
title('Similitud de P_std con el resto del grupo', 'Interpreter','none');
ylabel('Correlación media', 'Interpreter','none', 'FontSize', 14);
xlabel('Grupo', 'Interpreter','none');
grid on
...
% Correlación P_media: intragrupo e intergrupos
f = figure('Visible','off');
...
title('Correlación de P_media entre pacientes', 'Interpreter','none');
ylabel('Correlación', 'Interpreter','none', 'FontSize', 14);
xlabel('Grupo', 'Interpreter','none');
grid on

saveas(f, fullfile(carpeta_boxplots, 'boxplot_correlacion_P_media_intra_inter.png'));
close(f)

% Correlación P_std: intragrupo e intergrupos
f = figure('Visible','off');
...
title('Correlación de P_std entre pacientes', 'Interpreter','none');
ylabel('Correlación', 'Interpreter','none', 'FontSize', 14);
xlabel('Grupo', 'Interpreter','none');
grid on

saveas(f, fullfile(carpeta_boxplots, 'boxplot_correlacion_P_std_intra_inter.png'));
close(f)


%% 8) GUARDAR TABLAS

archivo_excel = fullfile(carpeta_out, 'tablas_correlaciones_ondaP.xlsx');
...
% Guardar en features_ondaP.xlsx una fila por paciente,
% incluyendo CorrGrupoMedia y CorrGrupoStd
archivo_features = fullfile(carpeta_out, 'features_ondaP.xlsx');
...
save(fullfile(carpeta_out, 'correlaciones_ondaP.mat'), ...
    'tabla_features', ...
...
fprintf('\nFIN SCRIPT DE TABLAS, HEATMAPS Y BOXPLOTS DE ONDA P\n');